...
class GurobiAlgorithm:
                ...
                for application_id in self.device_request(device_id, time):
                    ...
                    if (device_id, application_id) in mk_list:
                        for microservice_id in self.application_contain_microservice_ids(application_id):
                            ...
                            total_total_sum += self.find_variable_deploy(variable_x, device_id, application_id, microservice_id, n, microservice_base)* self.get_microservice_cpu(microservice_id)
                    else:
                        for microservice_id in self.application_contain_microservice_ids(application_id):
                            if microservice_id == 0:
                                # 0号微服务没有算力占用
    ...
    def Objective_function(self, gurobi_model, variable_x, m_list, k_list, epsilon:float = 0.01):
        ...
        for idx in range(len(m_list)):
            ...
            obj2 = [variable_x[i+variable_idx] * Matrix_W_k[i][j] * variable_x[j+variable_idx] for i in range(len(Matrix_W_k)) for j in range(len(Matrix_W_k))]
